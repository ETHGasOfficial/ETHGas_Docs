...
<details className="api-endpoint" id="post-builder-update-ofac">
<summary className="api-endpoint-header">
  <span className="api-method-post">**POST**</span> `/v1/builder/update/ofac` - Update OFAC flag for existing builder
</summary>
...
```bash
curl -H "Authorization: Bearer {{access_token}}" -X POST /v1/builder/update/ofac?publicKeys=0x12345...,0x2df345...&ofac=true
```
...
<details className="api-endpoint" id="post-builder-bundle-reject">
<summary className="api-endpoint-header">
  <span className="api-method-post">**POST**</span> `/v1/builder/bundle/reject/{slot}` - Reject bundles for a specific slot
</summary>
...
```bash
curl -H "Authorization: Bearer {{access_token}}" -X POST /v1/builder/bundle/reject/123 \
...
| └ uuid | Yes | string | Bundle UUID |
| └ rejectCode | Yes | integer | Rejection code |
| └ txHashList | Yes | string[] | List of transaction hashes |
| └ reason | Yes | string | Rejection reason |
...
<details className="api-endpoint" id="get-builder-bundle-reject">
<summary className="api-endpoint-header">
  <span className="api-method-get">**GET**</span> `/v1/p/builder/bundle/reject/{slot}/{builderAccountId}` - Get bundle rejection information for a specific slot and account
</summary>
